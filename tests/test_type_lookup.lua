...
function TestTypeLookup:TestLookupIntByHashstr()
   local t1_hstr = ffi.new("char["..tostring(ffi.C.UBX_TYPE_HASHSTR_LEN+1)..']')
...
function TestTypeLookup:TestLookupInvalidHash()
   local hash = ffi.new("char["..tostring(ffi.C.UBX_TYPE_HASH_LEN+1)..']')
   local t1 = ubx.type_get_by_hash(NI, hash)
   luaunit.assert_nil(t1, "invalid type lookup succeded")
end

function TestTypeLookup:TestLookupInvalidHashstr()
   local hashstr = ffi.new("char["..tostring(ffi.C.UBX_TYPE_HASHSTR_LEN+1)..']')
   for i=0,ffi.C.UBX_TYPE_HASHSTR_LEN-1 do hashstr[i] = string.byte('a') end
